...
--1)   *
SELECT 
    C.client_id AS Κωδικός, 
    C.afm AS ΑΦΜ, 
    C.client_name AS Όνομα, 
    C.adress AS Διεύθυνση, 
    T.phone AS Τηλέφωνο
FROM CLIENT C
...
--6) *
UPDATE CREDIT_CARD
SET card_epitokio = card_epitokio - 1.0; -- Υπόθεση: το επιτόκιο είναι σε ποσοστιαία μονάδα
...
SELECT 
    P.client_name ,
	P.surname ,
    MONTH(E.exchange_date) AS Μήνας, 
...
ORDER BY P.client_name,P.surname, Μήνας;
...
HAVING
 ...
 -- View για τις αγορές του Ιουνίου 2016
 GO
...
GO
--  View για τις αγορές του Ιουνίου 2017
GO
...
--  View για τις συνολικές αγορές ανά πελάτη το 2017
GO
...
GO
--  View για τις συνολικές πληρωμές ανά πελάτη το 2017
